...
public class Caminos {
		public static void main(String[] args) {
			...
			PositionList<PositionList<Vertex<Character>>> resultado = caminoEconomico(A, E, G);
			System.out.println("El camino es: ");
			
			for(PositionList<Vertex<Character>> p : resultado) {
				for(Vertex<Character> v :p) {
		...
		public static PositionList<PositionList<Vertex<Character>>> caminoEconomico (Vertex<Character> v1, Vertex<Character> v2, GraphD<Character,Integer> G){
		...
		private static void caminoEco2 (Vertex<Character> origen, Vertex<Character> destino, GraphD<Character,Integer> G, Map<Vertex<Character>, Boolean> visitados, Par2 caminoCorto, PositionList<Vertex<Character>> caminoAux, Integer pesoAux){
			...
			for(Edge<Integer> ed : G.succesorEdges(origen)) {
				...
				if(op == destino){
					caminoAux.addLast(op);
						...
						PositionList<Vertex<Character>>aux=(clonar(caminoAux));
						caminoCorto.getCamino().addLast(aux);					
						caminoAux.remove(caminoAux.last());
								}
				else {
					if(visitados.get(op) == false) {
						caminoEco2(op, destino, G, visitados, caminoCorto, caminoAux, pesoAux+ed.element());
					}
				}
			}
		...
		// Metodo auxiliar para clonar un camino
